...
if ($folder_id == 0){
	...
	echo("alert('文件區為最上層資料夾，無法修改。');");
	echo("history.back();");
	echo("</script>");
	exit();
}else{
	$str = "Select folder_name, folder_explain From document_folder Where teacher_id = '" . $teacher_id . "' And folder_id = '" . $folder_id . "'";
...
<HTML>
<HEAD>
<TITLE><? echo($homepage_title); ?> → 文件區 → 修改資料夾</TITLE>
</HEAD>
...
<FORM METHOD=POST ACTION="DocumentFolderEditDB.php">
<TABLE border="0">
<TR>
