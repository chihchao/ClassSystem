...
function show_folder($list_folder_id, $sub_tree){
	...
	if (is_array($folder_array[$list_folder_id])){
		...
		while(list($array_folder_id)  = each($folder_array[$list_folder_id])){
			...
			echo("<TD width=100%>");
			if (!$hidden_link) echo("<a href=PhotoFolderMoveDB.php?teacher_id=" . $teacher_id . "&folder_id=" . $folder_id . "&up_folder_id=" . $array_folder_id . ">");
...
if ($folder_id == 0){
	$folder_name = "相片區";
	$folder_echo_str = "『相片區』 為最上層資料夾，無法搬移。";
}else{
...
<HTML>
<HEAD>
<TITLE><? echo($homepage_title); ?> → 相片區 → 搬移資料夾</TITLE>
</HEAD>
