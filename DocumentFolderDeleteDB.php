...
function get_delete_str($delete_folder_id){
	...
	if (is_array($document_array[$delete_folder_id])){
		while (list($delete_document_id) = each($document_array[$delete_folder_id])){
			...
			if (!empty($document_array[$delete_folder_id][$delete_document_id])) $deletefile_array[$delete_document_id] = "UploadDocument/" . $document_array[$delete_folder_id][$delete_document_id];
		}
	}
}
...
if ($folder_id == 0){
	...
	$str = "Select up_folder_id From document_folder Where folder_id = '" . func_escape_string($folder_id) . "'";
	$result = QueryDatabase($str);
	list($up_folder_id) = mysql_fetch_row($result);
}

$folder_array = array();
$str = "Select up_folder_id, folder_id From document_folder Where teacher_id = '" . func_escape_string($teacher_id) . "'";
$result = QueryDatabase($str);
...
$str = "Select document_id, folder_id, document_file From document_document Where teacher_id = '" . func_escape_string($teacher_id) . "'";
$result = QueryDatabase($str);
...
$str = "Delete From document_folder Where" . $delete_folder_str;
QueryDatabase($str);
$str = "Delete From document_document Where" . $delete_document_str;
QueryDatabase($str);
header("location:Document.php?teacher_id=" . $teacher_id . "&folder_id=" . $up_folder_id);
?>