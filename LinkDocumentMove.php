...
function show_folder($list_folder_id){
	...
	if (is_array($folder_array[$list_folder_id])){
		...
		while(list($array_folder_id)  = each($folder_array[$list_folder_id])){
			...
			echo("<TD width=100%>");
			if ($array_folder_id != $folder_id) echo("<a href=LinkDocumentMoveDB.php?teacher_id=" . $teacher_id . "&document_id=" . $document_id . "&folder_id=" . $array_folder_id . ">");
...
$str = "Select folder_id, document_title From link_document Where teacher_id = '" . $teacher_id . "' And document_id = '" . $document_id . "'";
...
<HTML>
<HEAD>
<TITLE><? echo($homepage_title); ?> → 連結區 → 搬移連結</TITLE>
</HEAD>
...
<TR>
	<TD width="100%"><font color="#FFFFFF">搬 . 移 . 連 . 結</font></TD>
	<TD>
