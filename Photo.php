...
function router_folder_path($router_folder_path_str, $router_folder_id){
	...
	$path_str = $path_str . "<a href=\"Photo.php?teacher_id=" . $teacher_id . "&folder_id=" . $router_folder_id . "\">" . $folder_array[$router_folder_id]["folder_name"] . "</a>";
...
function show_folder($down_folder_id){
	...
	echo("</TD>");
	echo("<TD width=100%>");
	echo("<a href=\"Photo.php?teacher_id=" . $teacher_id . "&folder_id=" . $down_folder_id . "\">" . $folder_array[$down_folder_id]["folder_name"] . "</a>");
	echo("</TD>");
...
function show_document($document_id){
	...
	echo("<TR>");
	echo("<TD valign=top>");
	if (file_exists('UploadPhoto/tn_' . $document_id)) {
		echo('<img src="UploadPhoto/tn_' . $document_id . '">');
	} elseif ($document_array[$document_id]["document_file_flash"]) {
		echo("<img src=Images/PhotoFileFlash.png>");
	} else {
		echo("<img src=Images/PhotoFileImage.png>");
	}
	echo("</TD>");
	echo("<TD width=100%>");
	echo("<a href=\"PhotoDocument.php?teacher_id=" . $teacher_id . "&document_id=" . $document_id . "\">" . $document_array[$document_id]["document_title"] . "</a>");
	echo("</TD>");
	echo("<TD valign=top><img src=Images/PublicBlankSpace.gif border=0 width=125 height=0><BR><font size=1>");
...
$folder_array[0]["folder_name"] = "相片區";
$folder_array[0]["folder_time"] = "";
$str = "Select up_folder_id, folder_id, folder_name, folder_time From photo_folder Where teacher_id = '" . func_escape_string($teacher_id) . "'";
...
if ($folder_id == 0){
	$folder_explain = $homepage_title . "的相片區。";
}else{
	$str = "Select folder_explain From photo_folder Where folder_id = '" . func_escape_string($folder_id) . "'";
	$result = QueryDatabase($str);
...
$str = "Select document_id, document_title, document_file, document_time, document_file_flash From photo_document Where teacher_id = '" . func_escape_string($teacher_id) . "' And folder_id = '" . func_escape_string($folder_id) . "'";
$result = QueryDatabase($str);
while (list($array_document_id, $array_document_title, $array_document_file, $array_document_time, $array_document_file_flash) = mysql_fetch_row($result)){
	...
	$document_array[$array_document_id]["document_file_flash"] = $array_document_file_flash;
}
?>
<HTML>
<HEAD>
<TITLE><? echo($homepage_title); ?> → 相片區</TITLE>
</HEAD>
...
<TR>
	<TD width="100%"><font color="#FFFFFF">相 . 片 . 區</font></TD>
	<TD>
	<a href="Photo.php?teacher_id=<? echo($teacher_id); ?>&folder_id=<? echo($folder_array[$folder_id]["up_folder_id"]); ?>"><IMG SRC="Images/PublicFolderUpButton.gif" BORDER=0 ALT="上移一層"></a></TD>
	<TD>
	<a href="PhotoDocumentAdd.php?teacher_id=<? echo($teacher_id); ?>&folder_id=<? echo($folder_id); ?>"><IMG SRC="Images/PhotoDocumentAddButton.gif" BORDER=0 ALT="新增相片"></a></TD>
	<TD>
	<a href="PhotoFolderAdd.php?teacher_id=<? echo($teacher_id); ?>&folder_id=<? echo($folder_id); ?>"><IMG SRC="Images/PublicFolderAddButton.gif" BORDER=0 ALT="新增資料夾"></a></TD>
	<TD>
	<a href="PhotoFolderEdit.php?teacher_id=<? echo($teacher_id); ?>&folder_id=<? echo($folder_id); ?>"><IMG SRC="Images/PublicFolderEditButton.gif" BORDER=0 ALT="修改資料夾"></a></TD>
	<TD>
	<a href="PhotoFolderMove.php?teacher_id=<? echo($teacher_id); ?>&folder_id=<? echo($folder_id); ?>"><IMG SRC="Images/PublicFolderMoveButton.gif" BORDER=0 ALT="搬移資料夾"></a></TD>
	<TD>
	<a href="PhotoFolderDelete.php?teacher_id=<? echo($teacher_id); ?>&folder_id=<? echo($folder_id); ?>"><IMG SRC="Images/PublicFolderDeleteButton.gif" BORDER=0 ALT="刪除資料夾"></a></TD>
</TR>
</TABLE>
